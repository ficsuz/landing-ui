<template>
    <div class="page-wrapper">
        ...
        <el-table :data="items" v-loading="loading" class="w-full">
            ...
            <el-table-column prop="category" label="Kategoriya" width="160" />
            <el-table-column prop="date" label="Sana" width="140" />
            <el-table-column prop="status" label="Holat" width="120">
                <template #default="{ row }">
                    <el-tag :type="row.status === 'published' ? 'success' : 'info'" size="small">
                        {{ row.status === 'published' ? 'Nashr' : 'Qoralama' }}
                    </el-tag>
                </template>
            </el-table-column>
        ...
        <el-empty v-if="!loading && items.length === 0" description="Yangiliklar mavjud emas" class="py-16" />

        <el-dialog v-model="showDialog" title="Yangilik qo'shish" width="560px">
            <el-form label-position="top">
                ...
                <el-form-item label="Kategoriya"><el-input placeholder="Kategoriya" /></el-form-item>
