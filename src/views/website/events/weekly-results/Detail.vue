<template>
    <div v-loading="store.loading" class="bg-white min-h-[50vh]">
        <div v-if="item" class="py-12 md:py-20">
        ...
        <div v-else-if="!store.loading" class="py-24 text-center text-[#8a94a6]">
            {{ $t('common.noData') }}
        </div>
    </div>
</template>
...
const htmlContent = computed(() => resolveTranslation(item.value?.content))
